...
#### PROFILE MAP ####
     ...
     if(nrow(unique(datasetProfile %>%  select(`country (country)`))) > 1 ){    #checking if dataset has data in more than one country
       ...
       if(unique(datasetLayer %>% select(`state (state_province)`)) == "Alaska"){
         ...
         profileMap <- ggplot(data =  map_data("world2")) +  # Alaska-specific mapping
           ggMapBase + ggUSAFillBlue + ggRedX +
           coord_cartesian(xlim = c(184.6,231.4),
                           ylim = c(53,71)) +
           mapTitleProfile
   ...
   } else if(datasetLayer$`country (country)`[1] == "United States" &
             nrow(unique(datasetLayer %>% select(`state (state_province)`))) == 1 &
             datasetLayer$`state (state_province)`[1] != "Alaska"){  
...
#### LAYER MAP ####
   ...
   if(nrow(unique(datasetLayer %>% select(`country (country)`))) > 1 ){
     ...
     layerMap <- ggplot(data =  map_data('state') %>%
                          filter(region %in% datasestState)) +
       ggMapBase + ggStateFillBlue + ggRedX + mapTitleLayer
   } else { #datasets in Alaska
       ...
       if(unique(datasetLayer %>% select(`state (state_province)`)) == "Alaska"){
         ...
         profileMap <- ggplot(data =  map_data("world2")) +  # Alaska-specific mapping
           ...
           coord_cartesian(xlim = c(184.6,231.4),
                           ylim = c(53,71)) +
           mapTitleProfile
       } else { profileMap <- ggplot(data =  map_data('state')) + #any other US state 
       ggMapBase + ggStateFillBlue + ggRedX +
         mapTitleProfile
       }
     } else { shift360 <-  360 #use auto-scaling map code (for datasets in one country that is not the US)  
     profileMap <- ggplot(data =  map_data("world2")) + 
       ggMapBase + ggRedX + adjustedCoords +
       mapTitleProfile   #coord_map("ortho", orientation = c(90, 0, 0))
     }
   } else if(datasetLayer$`country (country)`[1] == "United States" &
             nrow(unique(datasetLayer %>% select(`state (state_province)`))) == 1 &
             datasetLayer$`state (state_province)`[1] != "Alaska"){
