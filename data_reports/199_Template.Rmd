...
plotGenerate <- function(ISCN3, datasetName){
  ...
  datasetState <- unique(datasetLayer$`state (state_province)`)   #use layer info to set state, and highlight state of interest on US map
  mapStateData <- map_data('state') %>% #pull out relevant info, found in state column
                  filter(region %in% datasetState)
  ...
  if(datasetName %in% ISCN3$profile$dataset_name_sub){    #checking if dataset has profile data
    if(nrow(unique(datasetProfile %>%  select(`country (country)`))) > 1 ){    #checking if dataset has data in more than one country
      profileMap <- ggplot(data =  map_data("world")) + ggMapBase + ggRedX + mapTitleProfile     # using world map
    } else if(datasetCountry ==  "United States"){   #checking if in USA to use USA map
      if(datasetState == "Alaska"){ shift360 <- 360     #shifting coordinates 360 degrees to correct orientation
      profileMap <- ggplot(data =  map_data("world2")) + ggMapBase + ggUSAFillBlue + ggRedX + alaskaCoords + mapTitleProfile
      } else { profileMap <- ggplot(data =  map_data('state')) + ggMapBase + ggStateFillBlue + ggRedX + mapTitleProfile # mapping for any other US state 
      }
    } else { shift360 <-  360 #use auto-scaling map code (for datasets in one country that is not the US)  
    profileMap <- ggplot(data =  map_data("world2")) + ggMapBase + ggRedX + adjustedCoords + mapTitleProfile 
    }
  } else if(datasetCountry ==  "United States" & datasetState != "Alaska"){  
    profileMap <- ggplot(data =  map_data('state')) + ggMapBase + ggStateFillBlue +
  ...
  if(nrow(datasetCountry) > 1 ){
    layerMap <-ggplot(data =  map_data('world')) + ggMapBase + ggRedX + mapTitleLayer    # using world map if sites span multiple countries
  } else if(datasetCountry ==  'United States' & na.omit(datasetState) != 'Alaska'){   # US state mapping 
    layerMap <- ggplot(data =  map_data('state') %>% filter(region %in% tolower(datasetState))) +  # reformatted state info to get corresponding map in ggplot data
      ggMapBase + ggStateFillBlue + ggRedX + mapTitleLayer
  } else { shift360 <- 360     # shifting coordinate system for Alaskan datasets
  layerMap <- ggplot(data =  map_data('world2')) + ggMapBase + ggUSAFillBlue + ggRedX + alaskaCoords + mapTitleLayer
  }
