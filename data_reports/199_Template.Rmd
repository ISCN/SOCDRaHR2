...
#### MAP CODE ####  
   ...
   if(datasetName %in% ISCN3$profile$dataset_name_sub) #checking if dataset has profile data
      { if (nrow(unique(datasetProfile %>%
       ...
       profileMap <- {baseLat <- datasetProfile %>%
                      dplyr::pull('lat (dec. deg)')
                   
                   baseLong <- datasetProfile %>%
                      dplyr::pull('long (dec. deg)')
                   
                   rangeLat <- if (diff(range(baseLong, na.rm = TRUE)) <= 10){
                      diff(range(baseLong, na.rm = TRUE)) * 12
                   } else {
                      diff(range(baseLong, na.rm = TRUE)) * 8
                   }
                   ...
                   #coord_map("ortho", orientation = c(90, 0, 0))
                               ...
                               nrow(unique(datasetLayer %>%
                                            select(`state (state_province)`))) == 1 &
...
if (nrow(unique(datasetLayer %>%
                        select(`country (country)`))) > 1 ) #using world map if sites span multiple countries
                    {
                      ...
                      layerMap <- ggplot(data =  map_data('state') %>% #pull out relevant info, found in state column
                                                 filter(region %in% datasestState)) +
                                      geom_polygon(aes(x=long, y = lat, group = group),
                                                   fill = 'grey', color = 'black') +
                                      geom_polygon(data = map_data('state') %>% #pull out relevant info, found in state column
                             ...
                             baseLat <- datasetLayer %>%
                               dplyr::pull('lat (dec. deg)')
                             
                             baseLong <- datasetLayer %>%
                               dplyr::pull('long (dec. deg)')
                             
                             if (diff(range(baseLong, na.rm = TRUE)) <= 20.0){
                                rangeLat <- diff(range(baseLong, na.rm = TRUE)) * 2
                             } else {
                                rangeLat <- diff(range(baseLong, na.rm = TRUE)) * 1
                             }
                             calcLat <- (rangeLat * 2.6)
