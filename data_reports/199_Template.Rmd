   ...
   if(datasetName %in% ISCN3$profile$dataset_name_sub){    #checking if dataset has profile data
     if(nrow(unique(datasetProfile %>%  select(`country (country)`))) > 1 ){    #checking if dataset has data in more than one country
       profileMap <- ggplot(data =  map_data("world")) +          #using world map
         geom_polygon(aes(x=long, y = lat, group = group), 
                      fill = 'grey', color = 'black') + 
         geom_point(data= datasetProfile, 
                    aes(x = `long (dec. deg)`, y = `lat (dec. deg)`),
                    shape = 'x', color = 'red') +
         coord_fixed(1.3) +
       ...
       if(unique(datasetLayer %>% select(`state (state_province)`)) == "Alaska"){
         profileMap <- ggplot(data =  map_data("world2")) +  # Alaska-specific mapping
           ...
           labs(title = 'Profile Data Map',
                caption = paste("A map of [", datasetName,
                                "] sampling sites,  \nusing coordinates found within the profile data.", sep = ""))
       } else { datasestState <- tolower(unique(datasetProfile$`state (state_province)`))   #use layer info to set state, and highlight state of interest on US map
     ...
     } else { baseLat <- datasetProfile %>%  #use auto-scaling map code (for datasets in one country that is not the US)
     ...
     profileMap <- ggplot(data =  map_data('state')) +
       geom_polygon(aes(x=long, y = lat, group = group),
                    fill = 'grey', color = 'black') +
       geom_polygon(data = map_data('state') %>% #pull out relevant info, found in state column
                      filter(region %in% datasestState),
                    aes(x=long, y = lat, group = group),
                    fill = 'light blue', color = 'black') +
       coord_fixed(1.3) +
     ...
     layerMap <-ggplot(data =  map_data('world')) +   #using world map if sites span multiple countries
       ...
       labs(title = 'Layer Data Map',
            caption = paste("A map of [", datasetName,
             ...
             nrow(unique(datasetLayer %>% select(`state (state_province)`))) == 1 &
             datasetLayer$`state (state_province)`[1] != "Alaska"){
     datasestState <- tolower(unique(datasetLayer$`state (state_province)`))
