...
# TODO Remove excess zeros from knitr kables
# TODO Remove first output "R Console"
# TODO Implement special case code


plotGen <- function(ISCN3, datasetName){
  
   dataset_profile <- ISCN3$profile %>%
                        filter(dataset_name_sub == datasetName) %>%
                        dplyr::select(where(function(xx){!all(is.na(xx))}))
   
   dataset_layer <- ISCN3$layer %>%
                        filter(dataset_name_sub == datasetName) %>%
                        dplyr::select(where(function(xx){!all(is.na(xx))}))
   
   dataset_study <- ISCN3$study %>%
                        filter(dataset_name == datasetName) %>%
                        dplyr::select(where(function(xx){!all(is.na(xx))}))
   
   type_columns <- ISCN3$type_columns
   
   
   studyTable <- knitr::kable(t(dataset_study), col.names = "" )
   
   profileTable <- knitr::kable(summary(dataset_profile %>%
                        select_if(is.factor))) 
      
   layerTable <- knitr::kable(summary(dataset_layer %>%
                        select_if(is.factor)))
   ...
   histograms <- ggplot(dataset_profile %>%
                              pivot_longer(cols = intersect(names(.), type_columns$num_cols), 
                                  values_to = 'measurement', names_to = 'type')) +
                              geom_histogram(aes(x=measurement)) +
                              facet_wrap(~type, scales='free') +
                          theme_bw()
   
   depthPlots <- ggplot(dataset_layer %>% 
                              pivot_longer(cols=c('layer_top (cm)', 'layer_bot (cm)'),
                                      values_to='depth') %>%
                              pivot_longer(cols = intersect(names(.), type_columns$num_cols), 
                                      values_to = 'measurement', names_to = 'type')) +
...
plotGen(ISCN3, datasetName = 'Heckman/Swanston Biscuit Burn')
...
## Study Table
knitr::kable(t(ISCN3$study %>%
        filter(dataset_name == datasetName)), col.names = "" )
...
## Layer Table
knitr::kable(summary(ISCN3$layer %>%
        filter(dataset_name == datasetName) %>% 
        select_if(is.factor)))
