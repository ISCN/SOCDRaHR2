...
The USGS Harden Yazoo data set in ISCN3 contains 234 layer-level rows and 35 profile-level information rows after cleaning for ISCN3.5.
...
3 columns of dataset_layer were removed from ISCN3 because they were determined using soc. (ASK ABOUT REMAINING SOC COLUMN)
...
if(any(grepl('ISCN', dataset_profile$dataset_name_soc))){
  ...
  dataset_profile <- dataset_profile %>%
    ...
    mutate(`country (country)` = 'United States', #hard-coding country as United States
           `site_note` = case_when(
             site_note == 'Harden et al., 1999' ~ 'Harden J, Fries T, Huntington T. 1999. MS Basin Carbon Project: Upland soil database for sites in Yazoo Basin, northern MS. USGS Open file report 99-319.',
             TRUE ~ site_note), #replacing cryptic site_note in two of the dataset_profile rows
           `site_note` = case_when(
             site_note == 'Harden et al., 1999; Huntington et al. 1998' ~ 'Harden J, Fries T, Huntington T. 1999. MS Basin Carbon Project: Upland soil database for sites in Yazoo Basin, northern MS. USGS Open file report 99-319; Huntington, T.G., Harden, J. W., Dabney, S. M. , Marion, D. A. , Alonso, C., Sharpe, J.M. , 1998. Soil, Environmental, and Watershed Measurements in support of carbon cycling studies in northwestern Mississippi. U.S. Geological Survey Open-File Report 98-501.',
             TRUE ~ site_note), #replacing remaining columns with full Harden 1999 and Huntington 1998 citations.
...
ggplot(data =  map_data("world")) + 
  geom_polygon(aes(x = long + 180, y = lat, group = group), 
               fill = 'grey', color = 'black') +  # + 180 added to ensure datapoints appeared in Missippi as intended
...
ggplot(data =  map_data("world")) + 
  ...
  coord_cartesian(xlim=c(80,110), ylim=c(20,40)) + #coord_cartesian values changed to focus in on southeastern United States
  theme_nothing() +
...
Please see @Harden1999 and @Huntington1998 for additional details and if you are using ISCN3 please cite.
