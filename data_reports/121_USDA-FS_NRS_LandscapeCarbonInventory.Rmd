...
The NAME HERE data set in ISCN3 contains 1510 layer-level rows and 741 profile-level information rows after cleaning for ISCN3.5.
...
datasetName <- "USDA-FS NRS Landscape Carbon Inventory" 
...
dataset_layer <- layer_raw %>%
  filter(dataset_name_sub == datasetName) %>%
  mutate(`country (country)` = 'United States')%>%
  mutate(`hzn`= NA_character_)%>%
  mutate(`hzn_desgn`= NA_character_)
...
Please see @Cole2013 for additional details and if you are using ISCN3 please cite.
