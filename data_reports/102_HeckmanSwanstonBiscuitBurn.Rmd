...
dataset_profile <- profile_raw %>%
  filter(dataset_name_sub ==  datasetName) %>% #pull out info from desired dataset
   mutate( #changing the following columns to remove ISCN computations
     `soc (g cm-2)` = if_else(grepl('ISCN', dataset_name_soc), #changing values in column "soc (g cm-2)" for if a row in "dataset_name_soc" contains "ISCN"....
                                  as.character(NA), #if TRUE, outputs "NA"
                              `soc (g cm-2)`), #if FALSE, remains unchanged
    `soc_carbon_flag` = if_else(grepl('ISCN', dataset_name_soc), #changing values in "soc_carbon_flag" when row contains "ISCN"
                                as.character(NA), #When row contains "ISCN", changes value to NA
                                `soc_carbon_flag`), #if not, value is unchanged
    `soc_method` = if_else(grepl('ISCN', dataset_name_soc), #searching "soc_method" for when a value contains "ISCN" in name, it will...
                           as.character(NA), #changes value to "NA"
                           `soc_method`)) %>% #returns original variable if it does not
  select(where(function(xx){!all(is.na(xx))})) %>% #only select values that are not "NA" then
  select(-dataset_name_soc)#column now redundant with dataset_name_sub
## subset the profile and dataset description tables and merge them into a study table
dataset_layer <- layer_raw%>%
  as.data.frame() %>% #creates a data frame from that table
  filter(dataset_name_sub != 'NRCS Sept/2014' & dataset_name_sub != ('ISCN 1-1 (2015-12-10)')) %>% 
   mutate( #changing the following columns to further remove ISCN computations
     `soc (g cm-2)` = if_else(grepl('ISCN', dataset_name_soc), #changing values in this column for if a row in "dataset_name_soc" contains "ISCN"....
                                  as.character(NA), `soc (g cm-2)`), #When row contains "ISCN", changes value to NA. If not, value is unchanged
    `soc_carbon_flag` = if_else(grepl('ISCN', dataset_name_soc), #changing values in this column when row contains "ISCN"
                                as.character(NA), `soc_carbon_flag`), #When row contains "ISCN", changes value to NA. If not, value is unchanged
    `soc_method` = if_else(grepl('ISCN', dataset_name_soc), 
                           as.character(NA), `soc_method`)) %>% #When this column contains "ISCN", change values to NA. If not, remain the same
  select(-dataset_name_soc)%>% #column now redundent with dataset_name_sub
  select(where(function(xx){!all(is.na(xx))})) #only selecting values that are not "NA"
