...
if(any(count(dataset_profile, dataset_name_sub, site_name, profile_name)$n > 1)){
  ...
  dataset_profile <- dataset_profile %>%
    ...
    mutate_at(vars(-group_cols()), 
              function(xx){ifelse(sum(!is.na(xx)) == 1, rep(xx[!is.na(xx)], length(xx)),xx)}) %>% #if there is one value that is not na then populate the rest of the entry, this fills in the
    ungroup() %>%
