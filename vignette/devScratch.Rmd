...
noNRCS_ISCN3_layer <- as.data.frame(ISCN_raw$layer[dataset_name_sub != 'NRCS Sept/2014', -c('ISCN 1-1 (2015-12-10)')])  %>% #remove NRCS contribution
...
type_cols <- list(num_cols  = c("lat (dec. deg)", "long (dec. deg)",
                  ...
                  char_cols = c("dataset_name", 
                                ...
                                "c_method", 'soc_method','bd_method', 'ph_method',
                                'wpg2_method',
                                'site_note', 'landform_note'),
...
standardCast <- function(data){
  ...
  mutate_at(intersect(type_cols$date_cols, names(.)), lubridate::as_date,
            origin = '1899-12-31') %>%
...
dataset_profile <- noNRCS_ISCN3_profile  %>%
  ...
  select(`dataset_name`, `site_name`, `profile_name`, 
         `lat (dec. deg)`, `long (dec. deg)`, `datum (datum)`, 
         `state (state_province)`, `country (country)`, 
         `observation_date (YYYY-MM-DD)`, `profile_zero_ref (profile_zero_ref)`, 
         #`layer_top (cm)`, `layer_bot (cm)`, 
         `soc_depth (cm)`, `soc (g cm-2)`, 
         `soil_taxon`, `soil_series`, 
         `elevation (m)`, `slope (percent)`, 
         `ecoregion`, `vegclass_local`, `surface_veg`, `landform (landform)`, `landform_note`,
         `map (mm)`, `mat (°C)`, `site_note`) 
