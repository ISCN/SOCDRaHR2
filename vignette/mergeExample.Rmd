...
ISCN.df <- read_delim(file = ISCN3Datafile, 
                    col_types = cols(.default =col_character()),
                    delim = ';', n_max = 1e4) %>% #row number = 445829 actually not a bad one-off runtime, a few minutes
  select("dataset_name_sub", "site_name", "profile_name", "layer_name", #ID columns
         #"soc (g cm-2)", "soc_carbon_flag", "soc_method",
         "bd_method", "bd_samp (g cm-3)", "bd_tot (g cm-3)", "bd_whole (g cm-3)", "bd_other (g cm-3)", "bdNRCS_prep_code")# SOC columns
...
ggplot(data_wide_values.df) +
  geom_histogram(aes(x=bulk_density_whole)) +
  facet_wrap(~study_id, scales='free_y') +
  scale_x_log10()

ggplot(data_wide_values.df %>%
         pivot_longer(cols = c(bulk_density_whole, bulk_density_sample, bulk_density_other))) +
