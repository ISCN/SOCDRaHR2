...
Pedon-level soils data from the NRCS-Soil Survey Lab, National Cooperative Soil Characterization Database (version date: 11 September 2014).
Sites are distributed across all 50 states, with some data also originating from U.S. Territories and foreign countries.
`NRCS Sept/2014` has 373326 layer-level and 26096 profile-level observations across 44896 and 13016 sites, respectively.
Please see @Nrcs2014a and @Nrcs2014b for additional details and if you are using ISCN3 please cite.
...
study <- ISCN3.ls$study %>% 
  #select current dataset from study table
  filter(dataset_name == current_dataset) %>%
...
profile <- ISCN3.ls$profile %>%
  #select current dataset from profile table
  filter(dataset_name_sub == current_dataset)%>% 
  #remove columns with no information
  select(where(~any(!is.na(.))))
...
if(any(ISCN3.ls$profile$dataset_name_sub == current_dataset)) { #if profile data exists, assign locations using profile data instead
  locations <- profile %>% 
    select(`lat (dec. deg)`, `long (dec. deg)`, `datum (datum)`) %>%
    bind_rows(locations) %>%
    unique() %>%
...
`Worldwide soil carbon and nitrogen data` has 21972 layer-level and 7423 profile-level observations across 1931 and 1835 sites, respectively.
...
This data set is located in the United States.
`USGS_S3C` has 10434 layer-level and 1674 profile-level observations across 1069 and 749 sites, respectively.
