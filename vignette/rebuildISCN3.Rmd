...
+ After cleaning for ISCN 3.3 this dropped to 372639 layer-level rows and 60895 profile-level rows.
+ In the profile tables 14756 rows were removed from ISCN3 that match the count of the `ISCN SOC stock to 1m computation` flag in the `dataset_name_soc` column implying that when that 1m computation was calculated, the other values in the ISCN contribution rows were duplicated from the other row in this orgnial dataset.
+ In the layer tables no rows were removed. 
+ Columns were with soil organic carbon density (volume and area) were removed in the profile and layer datasets since they were entirely filled in by ISCN.  
