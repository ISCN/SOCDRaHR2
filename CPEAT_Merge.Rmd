...
```{r}
#running loops for the three distinctive classes 

# loop for the Geochemistry classification of C-PEAT Data 
PAGES_Geochemistry_data <- data.frame()

# for (i in 1:nrow(PAGES_Geochemistry)) {
#   # using pg_data will automatically download the .tab format of the datasets (metadata + data) in the cache folder
#   all_Geochemistry_data <- pg_data(doi=PAGES_Geochemistry[i,2]) 
#   #print(pg_data(doi=PAGES_Geochemistry[i,2]))
#   print(all_Geochemistry_data)
#   print(i)
#   all_Geochemistry_data <- all_Geochemistry_data[[1]]$data 
#   if (is.character(all_Geochemistry_data$Peat)) {
#    all_Geochemistry_data <- rename(all_Geochemistry_data, `Peat(species)` = Peat)
#   }
#   # add columns identifying unique source of data for further attribution
#   all_Geochemistry_data$DOI <- as.character(PAGES_Geochemistry[i,2])
#   all_Geochemistry_data$citation <- as.character(PAGES_Geochemistry[i,5])
#   PAGES_Geochemistry_data <- as.data.frame(bind_rows(PAGES_Geochemistry_data, all_Geochemistry_data))
# }

write.table(PAGES_Geochemistry_data, file=paste0(datapath,"PAGES_Geochemistry_CPEAT.txt"), row.names = FALSE, quote = FALSE, sep = "\t", na = "")




allData.ls <- plyr::dlply(PAGES_Geochemistry[1:nrow(PAGES_Geochemistry),], c("doi"), .fun = function(xx) {
  all_Geo_data <- pg_data(doi = xx$doi, 
...
allMeta.df <- plyr::ldply(allData.ls, .fun = function(xx) {
  characterNames <- intersect(names(xx), c('parent_doi', 'doi', 'citation', 'url', 'path'))
  ans <- as.data.frame(xx[characterNames], stringsAsFactors = FALSE)
  # add columns identifying unique source of data for further attribution
  return(ans)
})

# try using one dataset as a test; merging the parameters 
test <- data.frame(header = names(allData.ls[['10.1594/PANGAEA.928065']]$data), description = unlist(lapply(allData.ls[['10.1594/PANGAEA.928065']]$metadata$parameters, paste, collapse = " ")), 
                    allData.ls[['10.1594/PANGAEA.928065']]$doi)

allParameters.df <-  plyr::ldply(allData.ls, .fun = function(xx) {
             ParametersGeo <- data.frame(header = names(xx$data), 
            description = unlist(lapply(xx$metadata$parameters, paste, collapse = " "))) #%>%
               #group_by(xx$doi) %>%
             #mutate(ParametersGeo = cur_group_id())  #this is not working as data masking verbs such as 'group-by' is missing
                    #xx$doi
})

# if we were to index the parameters of respective datasets; use "arrange" function?
